...
import { JSXLogicalExpressionContainer, JSXConditionalExpressionContainer } from "../converters/predicates/react/JSXLogicalExpression";
...
class ResolversRegistry {
  ...
  constructor() {
    this.resolvers = [
      ...
      new JSXLogicalExpressionContainer(),
      new JSXConditionalExpressionContainer(),
    ];
  }
  ...
  resolve(input: any) {
    ...
    if (!resolvers.length)
      throw new Error(`Unable to resolve type ${input.type}`);
    const finalResolver = resolvers.find(x => x.isMatching(input));
    if (!finalResolver)
      throw new Error(
        `No resolver found to match current input: ${input.type}`
      );
